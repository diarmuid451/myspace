...
public class UploadFileAction implements Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			e.printStackTrace();	//개발 중에 필요한 에러 확인 용
	...
	if (formItems != null && formItems.size() > 0) {
		for (FileItem item : formItems) { // form items 반복하여 꺼내는 구문
			if (!item.isFormField()) {	// 파일일 경우 해당-업로드된 파일 저장		
				// uuid+구분자+파일명
				fileName = MakeFileName.toUUIDFileName(".*", "");
				...
				case "oldPicture":
					String oldFilName = item.getString("utf-8");
					File oldFile = new File(uploadPath+File.separator+oldFilName);
					if (oldFile.exists()) {
						oldFile.delete();
					}
					break;
